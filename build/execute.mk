...
RTT_BUILD_DIR := .
BSP_BUILD_DIR := output

#################

define add_c_file
$(eval C_SRC := $(1:$(BSP_ROOT)/%=$(BSP_ROOT)/%)) \
$(eval C_SRC := $(C_SRC:$(RTT_ROOT)/%=$(RTT_ROOT)/%)) \
$(eval COBJ := $(1:%.c=%.o)) \
...
define add_cxx_file
$(eval CXX_SRC := $(1:$(BSP_ROOT)/%=$(BSP_ROOT)/%)) \
$(eval CXX_SRC := $(CXX_SRC:$(RTT_ROOT)/%=$(RTT_ROOT)/%)) \
$(eval CXXOBJ := $(1:%.cpp=%.o)) \
...
define add_s_file
$(eval s_SRC := $(1:$(BSP_ROOT)/%=$(BSP_ROOT)/%)) \
$(eval s_SRC := $(s_SRC:$(RTT_ROOT)/%=$(RTT_ROOT)/%)) \
$(eval sOBJ := $(1:%.s=%.o)) \
...
$(TARGET): $(OBJS) $(EXTERN_LIB)
	...
	@echo ------------------------------------------------
	@$(CROSS_COMPILE)objcopy -O binary $@ $(TARGET_NAME).bin
	@$(CROSS_COMPILE)size $@ 
endif
...
clean:
	@echo clean
	@rm -rf $(TARGET) $(TARGET_NAME).bin $(TARGET_NAME).map $(BUILD_DIR)/output 
